...
static void doorbell_cmd_set_keepalive(int fd)
{
	int opt = 1;
	// open tcp keepalive
	setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &opt, sizeof(int));

	opt = 30;  // 5 second
	setsockopt(fd, IPPROTO_TCP, TCP_KEEPIDLE, &opt, sizeof(int));

	opt = 1;  // 1s second for intval
	setsockopt(fd, IPPROTO_TCP, TCP_KEEPINTVL, &opt, sizeof(int));

	opt = 3;  // 3 times
	setsockopt(fd, IPPROTO_TCP, TCP_KEEPCNT, &opt, sizeof(int));
}
		...
		else
		{
			...
			if (FD_ISSET(db_cmd_info->server_fd, &watchfd))
			{
				...
				while (db_cmd_info->server_state == BK_TRUE)
				{
					...
					{
						// close this socket
						LOGI("recv close fd:%d, rcv_len:%d, error:%d\n", db_cmd_info->client_fd, rcv_len, errno);
