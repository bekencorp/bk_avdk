...
int doorbell_tcp_img_send_packet(uint8_t *data, uint32_t len, uint16_t *retry_cnt)
{
	...
	return doorbell_socket_sendto(&db_tcp_service->img_fd, (struct sockaddr *)&db_tcp_service->img_remote, data, len, -sizeof(db_trans_head_t), retry_cnt);
}
...
int doorbell_tcp_aud_send_packet(uint8_t *data, uint32_t len, uint16_t *retry_cnt)
{
	...
	return doorbell_socket_sendto(&db_tcp_service->aud_fd, (struct sockaddr *)&db_tcp_service->aud_remote, data, len, -sizeof(db_trans_head_t), retry_cnt);
}
...
void doorbell_tcp_service_init(void)
{
	...
	db_tcp_service->img_channel = doorbell_transmission_malloc(0, 1460);
	db_tcp_service->aud_channel = doorbell_transmission_malloc(1460, 1460);
