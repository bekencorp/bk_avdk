...
{
	os_printf("audio_record_to_sdcard {start|stop file_name} \r\n");
}
...
void cli_audio_record_to_sdcard_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		if (BK_OK != audio_record_to_sdcard_start(argv[2]))
...
{
	{"audio_record_to_sdcard", "audio_record_to_sdcard {start|stop file_name}", cli_audio_record_to_sdcard_cmd},
};
