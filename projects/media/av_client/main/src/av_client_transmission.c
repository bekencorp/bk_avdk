...
void av_client_hex_dump(uint8_t *data, uint32_t length)
{
	for (int i = 0; i < 12; i++)
	{
...
void av_client_transmission_unpack(db_channel_t *channel, uint8_t *data, uint32_t length, av_client_transmission_recive_cb_t cb)
{
	...
	av_client_hex_dump(data, length);
#else
	LOGD("recv unpack: %u\n", length);
#endif
	...
	while (left != 0)
	{
		...
		else
		{
			...
			else
			{
				...
				av_client_hex_dump(channel->cbuf, channel->ccount);
				//TODO FIXME
				break;
			}
...
uint32_t av_client_transmission_get_milliseconds(void)
{
	uint32_t time = 0;
...
void av_client_transmission_pack(db_channel_t *channel, uint8_t *data, uint32_t length)
{
	...
	*/
	...
	head->timestamp = CHECK_ENDIAN_UINT32(av_client_transmission_get_milliseconds());
...
int av_client_transmission_pack_send(db_channel_t *channel, uint8_t *data, uint32_t length, av_client_transmission_send_t cb)
{
	...
	av_client_transmission_pack(channel, data, length);
