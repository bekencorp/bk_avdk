...
int doorbell_udp_img_send_packet(uint8_t *data, uint32_t len, uint16_t *retry_cnt)
{
	...
	return doorbell_socket_sendto(&db_udp_service->img_fd, (struct sockaddr *)&db_udp_service->img_remote, data, len, -sizeof(db_trans_head_t), retry_cnt);
}
...
int doorbell_udp_aud_send_packet(uint8_t *data, uint32_t len, uint16_t *retry_cnt)
{
	...
	return doorbell_socket_sendto(&db_udp_service->aud_fd, (struct sockaddr *)&db_udp_service->aud_remote, data, len, -sizeof(db_trans_head_t), retry_cnt);
}
