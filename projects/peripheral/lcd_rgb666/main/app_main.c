...
#define CMDS_COUNT  (sizeof(s_rgb666_lcd_commands) / sizeof(struct cli_command))

void cli_rgb666_lcd_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	os_printf("%s\r\n", __func__);
}

static const struct cli_command s_rgb666_lcd_commands[] =
{
	{"rgb666_lcd", "rgb666_lcd", cli_rgb666_lcd_cmd},
};

int cli_rgb666_lcd_init(void)
{
	return cli_register_commands(s_rgb666_lcd_commands, CMDS_COUNT);
}
...
static int bk_rgb666_lcd_example_init(uint8_t *data, uint32_t size, void *param)
{
#if (CONFIG_SYS_CPU1)
    int ret;
...
#if (CONFIG_SYS_CPU0)
void bk_rgb666_lcd_main(void)
{
	cli_rgb666_lcd_init();

    bk_err_t ret;

    os_printf("!!!BK7258 LCD RGB666 DISPLAY EXAMPLE!!!\r\n");
...
int main(void)
{
    ...
    cfg.cb = bk_rgb666_lcd_example_init;
    cfg.name = "app";
    cfg.param = NULL;
