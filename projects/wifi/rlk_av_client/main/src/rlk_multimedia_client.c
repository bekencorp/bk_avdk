...
int rlk_mm_client_video_send_packet(uint8_t *data, uint32_t len)
{
    int send_byte = BK_OK;
    uint8_t *media_data;
...
int rlk_mm_client_voice_send_packet(uint8_t *data, uint32_t len)
{
    int send_byte = BK_OK;
    uint8_t *media_data;
    rlk_mm_pre_header_t *header = NULL;
...
bk_err_t rlk_mm_client_init(void)
{
    ...
    /*********************< voice init end >**************************/
   ret = rlk_mm_client_video_aud_init();

    if (ret != BK_OK)
    {
        LOGE("rlk_mm_client_video_aud_init failed\n");
        goto error;
    }
