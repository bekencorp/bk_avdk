...
beken2_timer_t rlk_cntrl_client_timer = {0};
beken2_timer_t rlk_client_wake_dur_timer = {0};
...
void bk_rlk_client_mgmt_tx_callback(void *mgmt_args, bool success)
{
    struct rlk_msg_t msg;
    bk_err_t ret = BK_OK;
...
static void rlk_cntrl_client_timer_handler(void *Larg, void *Rarg)
{
    uint32_t time = 0;
    struct rlk_msg_t msg;
    ...
    rtos_start_oneshot_timer(&rlk_cntrl_client_timer);
}

///ask rlk client go to sleep only from cli
void rlk_client_ps_cmd_handler(void)
{
    rtos_stop_oneshot_timer(&rlk_cntrl_client_timer);
    bk_rlk_sleep();
...
void rlk_client_wake_duration_end_handler(void *Larg, void *Rarg)
{
    if (rlk_client_local_env.state == RLK_STATE_SLEEP)
    {
        rtos_stop_oneshot_timer(&rlk_cntrl_client_timer);
        rtos_stop_oneshot_timer(&rlk_client_wake_dur_timer);
        bk_rlk_sleep();
        rlk_client_local_env.state = RLK_STATE_SLEEP;
        rlk_client_rtc_sleep_start(PM_MODE_LOW_VOLTAGE, RLK_LP_RTC_PS_INTERVAL, PM_SLEEP_MODULE_NAME_APP, 0, 0, 0);
    }
...
bk_err_t rlk_cntrl_client_tx_mgmt_data(const uint8_t *mac_addr, uint8_t msg_type)
{
    ...
    rlk_cntrl.data =(uint8_t *)(&header);
    rlk_cntrl.len = sizeof(header);
    rlk_cntrl.cb = bk_rlk_client_mgmt_tx_callback;
    rlk_cntrl.args = NULL;

    ret = bk_rlk_send_ex(mac_addr, &rlk_cntrl);
...
bk_err_t rlk_cntrl_client_init(void)
{
    ...
    //Configure the MAC header type by BK-RLK
    BK_LOG_ON_ERR(bk_rlk_set_mac_hdr_type(WIFI_MAC_HDR_TYPE_ACTION));
    ...
    // set send rate
    BK_LOG_ON_ERR(bk_rlk_set_tx_rate(WIFI_LEGACY_RATE_36MBPS));
...
{
    bk_rlk_wakeup();
    rtos_start_oneshot_timer(&rlk_client_wake_dur_timer);
}
...
bk_err_t rlk_client_init(void)
{
    ...
    rtos_init_oneshot_timer(&rlk_cntrl_client_timer, RLK_KEEPALIVE_TIME_INTERVAL, (timer_2handler_t)rlk_cntrl_client_timer_handler, NULL, NULL);
    rtos_init_oneshot_timer(&rlk_client_wake_dur_timer, RLK_LP_WAKEUP_DURATION, (timer_2handler_t)rlk_client_wake_duration_end_handler, NULL, NULL);
