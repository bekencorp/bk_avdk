...
beken2_timer_t rlk_cntrl_server_timer = {0};
...
static void rlk_cntrl_server_timer_handler(void *Larg, void *Rarg)
{
    ...
    rtos_start_oneshot_timer(&rlk_cntrl_server_timer);
}
...
bk_err_t rlk_cntrl_server_tx_mgmt_data(const uint8_t *mac_addr, uint8_t msg_type)
{
    ...
    rlk_cntrl.cb = bk_rlk_server_mgmt_tx_callback;
    rlk_cntrl.args = NULL;
...
bk_err_t rlk_server_init(void)
{
    ...
    rtos_init_oneshot_timer(&rlk_cntrl_server_timer, RLK_KEEPALIVE_TIME_INTERVAL, (timer_2handler_t)rlk_cntrl_server_timer_handler, NULL, NULL);
