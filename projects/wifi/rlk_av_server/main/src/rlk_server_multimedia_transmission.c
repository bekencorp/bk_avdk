...
void mm_hex_dump(uint8_t *data, uint32_t length)
{
    for (int i = 0; i < 10; i++)
    {
    ...
    while (left != 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                mm_hex_dump(channel->cbuf, channel->ccount);
                //TODO FIXME
                break;
            }
...
uint32_t mm_transmission_get_milliseconds(void)
{
    uint32_t time = 0;
...
void mm_transmission_pack(db_channel_t *channel, uint8_t *data, uint32_t length)
{
    ...
    */
    ...
    head->timestamp = CHECK_ENDIAN_UINT32(mm_transmission_get_milliseconds());
...
int mm_transmission_pack_send(db_channel_t *channel, uint8_t *data, uint32_t length, mm_transmission_send_t cb)
{
    ...
    mm_transmission_pack(channel, data, length);
