...
static s32 _dec_img(frame_buffer_t *jpeg_frame, lv_img_dsc_t *img_dst)
{
    ...
    do{
        ...
        ret = bk_jpeg_dec_sw_start(JPEGDEC_BY_FRAME, jpeg_frame->frame, (uint8_t *)img_dst->data, jpeg_frame->length, 0, &result);
        if (ret != BK_OK)
        {
...
static frame_buffer_t *_read_file_from_sdcard(char *file_name)
{
    ...
    do{
        file_len = bk_read_sdcard_file_length(file_name);
        if(file_len <= 0)
        {
        ...
        ret = bk_sdcard_read_to_mem((char *)file_name, (uint32_t *)jpeg_frame->frame, &jpeg_frame->length);
        if(BK_OK != ret)
        {
