...
const char * _lv_demo_music_get_title(uint32_t track_id)
{
    ...
    if (track_id > audio_tf_get_mp3_files_count()) return NULL;
	return tf_mp3_file[track_id].fname;
...
uint32_t _lv_demo_music_get_track_length(uint32_t track_id)
{
    ...
    if(track_id > audio_tf_get_mp3_files_count()) return 0;
    return (tf_mp3_file[track_id].fsize / 16000);
}
