...
static void timer_cb(lv_timer_t * t)
{
    LV_UNUSED(t);
    time_act++;
	// os_printf("song_current_time: %d\n\r", audio_player_get_song_rate());
	...
	//audio_player_get_song_rate
	lv_label_set_text_fmt(time_obj, "%"LV_PRIu32":%02"LV_PRIu32, audio_player_get_song_rate() / 60, audio_player_get_song_rate() % 60);
	lv_slider_set_value(slider_obj, audio_player_get_song_rate(), LV_ANIM_ON);
}
