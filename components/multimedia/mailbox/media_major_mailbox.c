		...
		else if(msg->type == MAILBOX_MSG_TYPE_NOTIFY)
		{
			switch (msg->event)
			{
				case EVENT_JPEG_DEC_START_COMPLETE_NOTIFY:
				{
					...
					if (msg->param != 0)
					{
						decode_callback_t cb = (decode_callback_t)msg->param;
						cb(msg->result);
					}
					else
					{
				...
				break;
                case EVENT_YUV_ROTATE_COMPLETE_NOTIFY:
                {
#if CONFIG_MEDIA_PIPELINE
                    if (msg->param != 0)
                    {
                        rotate_callback_t cb = (rotate_callback_t)msg->param;
                        cb(msg->result);
                    }
                    else
                    {
                        LOGE("%s rotate complete ret is %d but no callback\n", __func__, ret);
                    }
#endif
                }
                break;
