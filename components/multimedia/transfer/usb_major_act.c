...
static void usb_major_task_transfer_start_handle(uint32_t param)
{
	...
	frame_buffer_fb_register(usb_trs_info.node, MODULE_WIFI);
...
static void usb_major_task_transfer_data_handle(uint32_t param)
{
	...
	{
		encode_frame = frame_buffer_fb_read(usb_trs_info.node, MODULE_WIFI, 50);
		...
		frame_buffer_fb_read_free(usb_trs_info.node, encode_frame, MODULE_WIFI);
	}
}
...
static void usb_major_task_exit_handle(uint32_t param)
{
	...
	frame_buffer_fb_deregister(usb_trs_info.node, MODULE_WIFI);
