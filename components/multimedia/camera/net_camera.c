...
const net_camera_config_t net_camera_config = {
    ...
    .frame_deinit = frame_buffer_list_node_deinit,
    .frame_clear = frame_buffer_list_node_invalid,
    .frame_malloc = frame_buffer_fb_malloc,
    .frame_complete = frame_buffer_fb_push,
    .frame_free = frame_buffer_fb_free,
    .stream = NULL;
};
...
static bk_err_t net_camera_free_memory(void)
{
	if (net_camera_buf)
	{
		...
		{
			net_camera_config.fb_free(net_camera_buf->frame);
...
static void net_camera_process_packet(uint8_t *data, uint32_t length)
{
	if ((net_camera_buf->start_buf == BUF_STA_INIT || net_camera_buf->start_buf == BUF_STA_COPY) && net_camera_buf->frame)
	{
			...
			&& (net_camera_buf->start_buf == BUF_STA_COPY))
		{
			...
			if (hdr->is_eof == 1)
			{
				net_camera_config.fb_complete(net_camera_buf->frame);

				net_camera_buf->frame = net_camera_config.fb_malloc(FB_INDEX_JPEG, 200 * 1024);
...
bk_err_t bk_net_camera_open(media_camera_device_t *device)
{
	...
	net_camera_config.stream = net_camera_config.frame_init(device->port, device->type, IMAGE_MJPEG);
	if (net_camera_config.stream == NULL)
	{
		LOGW("%s, create stream fail\n", __func__);
		goto error;
	}

	net_camera_buf->frame = net_camera_config.frame_malloc(net_camera_config.stream, CONFIG_JPEG_FRAME_SIZE);
	if (net_camera_buf->frame == NULL)
	{
		goto error;
	}

	net_camera_buf->frame->fmt = device->fmt;
