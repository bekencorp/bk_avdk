...
static void storage_save_every_frame_handle(void)
{
	frame_buffer_t *frame = NULL;
...
static void storage_save_video_start_handle(void)
{
	while (storage_task_running)
	{
		storage_save_every_frame_handle();
	};

	rtos_set_semaphore(&storage_save_sem);
}

static void storage_save_video_stop_handle(void)
{
	LOGI("%s\n", __func__);

	if (storage_info.capture_state == STORAGE_STATE_DISABLED)
	{
		return;
	}
...
static void storage_task_exit(void)
{
	storage_save_video_stop_handle();
...
static void storage_task_entry(beken_thread_arg_t data)
{
	...
	storage_info.state = STORAGE_STATE_ENABLED;
		...
		if (kNoErr == ret)
		{
			switch (msg.type)
			{
				...
				case STORAGE_TASK_SAVE_START:
					storage_save_video_start_handle();
					break;
...
exit:
	...
	rtos_deinit_semaphore(&storage_save_sem);
	storage_save_sem = NULL;
...
int storage_task_start(void)
{
	...
	if (storage_save_sem == NULL)
	{
		ret = rtos_init_semaphore_ex(&storage_save_sem, 1, 0);
...
void storage_open_handle(param_pak_t *param)
{
	int ret = BK_OK;
	LOGI("%s\n", __func__);

	if (storage_info.state == STORAGE_STATE_ENABLED)
	{
		LOGI("storage task have already opened!\r\n");
		goto out;
	}

	ret = storage_task_start();

out:
	MEDIA_EVT_RETURN(param, ret);
}

void storage_close_handle(param_pak_t *param)
{
	LOGI("%s\n", __func__);

	if (storage_info.state == STORAGE_STATE_DISABLED)
	{
		LOGI("storage task have already closed!\r\n");
		goto out;
	}
	...
	while (storage_info.state == STORAGE_STATE_ENABLED)
	{
...
void storage_save_start_handle(param_pak_t *param)
{
	LOGI("%s, %s\n", __func__, (char *)param->param);
	...
	storage_task_send_msg(STORAGE_TASK_SAVE_START, 0);

out:
	MEDIA_EVT_RETURN(param, kNoErr);
}

void storage_save_stop_handle(param_pak_t *param)
{
	LOGI("%s\n", __func__);
	...
	storage_task_send_msg(STORAGE_TASK_SAVE_STOP, 0);

	while (storage_info.capture_state == STORAGE_STATE_ENABLED)
	{
		rtos_delay_milliseconds(20);
	}
...
void storage_event_handle(uint32_t event, uint32_t param)
{
	switch (event)
	{
		case EVENT_STORAGE_OPEN_IND:
			storage_open_handle((param_pak_t *)param);
			break;
		...
		case EVENT_STORAGE_SAVE_START_IND:
			storage_save_start_handle((param_pak_t *)param);
			break;

		case EVENT_STORAGE_SAVE_STOP_IND:
			storage_save_stop_handle((param_pak_t *)param);
			break;

		case EVENT_STORAGE_CLOSE_IND:
			storage_close_handle((param_pak_t *)param);
			break;
