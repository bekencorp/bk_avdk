...
int pipeline_on_off_test(int argc, char **argv, int delay, int times)
{
	...
	for (i = 0; i < times; i++)
	{
		...
		if (ut_check_part(1, 1, 1, 0, 0, 0) == false)
		{
		...
		ret = media_app_register_read_frame_callback(PIXEL_FMT_H264, ut_read_frame_callback);
		if (ret == BK_OK)
		{
			LOGI("read h264 open success\n");
		}
		else
		{
			LOGI("read h264 open failed\n");
			goto error;
		}

		rtos_delay_milliseconds(delay);

		if (ut_check_part(1, 1, 1, 1, 1, 0) == false)
		{
			media_app_pipeline_dump();
			break;
		}
		...
		if (ret == BK_OK)
		{
			LOGI("read h264 close success\n");
		}
		else
		{
			LOGI("read h264 close failed\n");
			goto error;
		}
