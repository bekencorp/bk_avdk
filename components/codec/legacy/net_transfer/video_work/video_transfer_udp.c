...
static int rtsp_udp_port = 0;
static uint32_t rtsp_udp_addr = 0;
...
typedef struct rtp_udp_header_t {
	union {
		struct 
		{
			uint8_t csrc_cnt:		4;
...
{
	rtp_udp_header_t *rtp_udp_header = (rtp_udp_header_t*)param->ptk_ptr;
	rtp_udp_header->ctrl1.v = 0x80;
	rtp_udp_header->ctrl2.type = 0x60;
	rtp_udp_header->ssrc = WORD_REVERSE(0x12345678);
	rtp_udp_header->ctrl2.mark = param->is_eof;
	pkt_seq ++;
	rtp_udp_header->seq_num = HB_REVERSE(pkt_seq);
	rtp_udp_header->time_stamp = WORD_REVERSE(time_s);
	if (param->is_eof) {
		time_s += 3600;
	}
	rtp_udp_header->ssrc = WORD_REVERSE(0x12345678);
...
static void app_demo_udp_main(beken_thread_arg_t data)
{
	...
	app_demo_remote->sin_port = htons(8080);
...
#if CONFIG_RTSP_SERVER
	app_demo_remote->sin_addr.s_addr = htonl(rtsp_udp_addr);
	app_demo_remote->sin_port = htons(rtsp_udp_port);
	UINT8 *src_ipaddr = (UINT8 *)&app_demo_remote->sin_addr.s_addr;
	os_printf("src_ipaddr: %d.%d.%d.%d\r\n", src_ipaddr[0], src_ipaddr[1],
						src_ipaddr[2], src_ipaddr[3]);
	os_printf("udp connect to new port:%d\r\n", app_demo_remote->sin_port);
	...
	setup.pkt_header_size = sizeof(rtp_udp_header_t);
