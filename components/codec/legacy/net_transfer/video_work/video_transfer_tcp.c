...
typedef struct rtp_tcp_header_t {
	uint8_t tcp_head_magic_num;
	uint8_t tcp_head_channel;
	uint16_t tcp_head_pack_len;
...
extern int demo_rtsp_tcp_video_send_packet(uint8_t *data, uint32_t len);
...
static void app_demo_add_rtp_header(video_packet_t *param)
{
	rtp_tcp_header_t *rtp_tcp_header = (rtp_tcp_header_t *)param->ptk_ptr;
	rtp_tcp_header->tcp_head_magic_num = 0x24;
	rtp_tcp_header->tcp_head_channel = 0x0;
	rtp_tcp_header->tcp_head_pack_len = HB_REVERSE(12 + 2 + param->ptklen);
	rtp_tcp_header->ctrl1.v = 0x80;
	rtp_tcp_header->ctrl2.type = 0x60;
	rtp_tcp_header->ssrc = WORD_REVERSE(0x12345678);
	rtp_tcp_header->ctrl2.mark = param->is_eof;
	pkt_seq ++;
	rtp_tcp_header->seq_num = HB_REVERSE(pkt_seq);
	rtp_tcp_header->time_stamp = WORD_REVERSE(time_s);
	if (param->is_eof) {
		time_s += 3600;
	}
	rtp_tcp_header->ssrc = WORD_REVERSE(0x12345678);
...
void app_demo_rtsp_start(void)
{
	video_setup_t setup;

	setup.open_type = TVIDEO_OPEN_RTSP;
	setup.send_type = TVIDEO_SND_TCP;
	setup.send_func = demo_rtsp_tcp_video_send_packet;
	setup.start_cb = app_demo_tcp_app_connected;
	setup.end_cb = app_demo_tcp_app_disconnected;
	setup.device = get_camera_device();

	setup.pkt_header_size = sizeof(rtp_tcp_header_t);
	setup.add_pkt_header = app_demo_add_rtp_header;;
