...
static media_transfer_cb_t doorbell_tcp_callback = DEFAULT_VIDEO_TRAS_CONFIG();
static media_camera_device_t camera_device = DEFAULT_CAMERA_CONFIG();
...
int demo_doorbell_tcp_video_send_packet(uint8_t *data, uint32_t len, uint16_t *retry_cnt)
{
	int i = 0, snd_len = 0;
	...
	if (data[0] == DOORBELL_CMD_IMG_HEADER)
	{
		...
		switch (data[1])
		{
			case DOORBELL_DVP_START:
			{
				...
				doorbell_tcp_callback.send = demo_doorbell_tcp_video_send_packet;
				camera_device.type = DVP_CAMERA;
				camera_device.info.resolution.width = ppi >> 16;
				camera_device.info.resolution.height = ppi & 0xFFFF;
				camera_device.info.fps = FPS25;
				...
				doorbell_tcp_callback.fmt = camera_device.fmt;
				bk_wifi_transfer_frame_open(&doorbell_tcp_callback);
			}
			break;
			...
			case DOORBELL_UVC_START:
			{
				...
				media_app_camera_open(&camera_device);
#if (!CONFIG_SOC_BK7256XX)
				if (camera_device.fmt == PIXEL_FMT_H264 && camera_device.mode == JPEG_MODE)
				{
