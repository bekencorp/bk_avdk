...
#if 0
	bk_uart_write_bytes(UART_ID_2, aec_info_pr->mic_addr, aec_info_pr->samp_rate_points*2);
#endif
...
static bk_err_t aud_tras_enc(void)
{
	...
	if (aud_tras_drv_info.aud_tras_tx_mic_data != NULL) {
		...
		if (size > 0) {
#if CONFIG_AUD_TRAS_LOST_COUNT_DEBUG
			aud_tras_drv_info.voc_info.lost_count.complete_size += size;
#endif
		}
	}
#endif
...
{
	uint32_t complete = aud_tras_drv_info.voc_info.lost_count.complete_size;
...
static bk_err_t aud_tras_drv_mic_tx_data(void)
{
	...
	if (ring_buffer_get_fill_size(&(aud_tras_drv_info.mic_info.mic_rb)) >= aud_tras_drv_info.mic_info.frame_size) {
		...
		flush_dcache((void *)aud_tras_drv_info.mic_info.mic_ring_buff, aud_tras_drv_info.mic_info.frame_size * 2);
#endif
...
static bk_err_t aud_tras_drv_spk_req_data(audio_packet_t * packet)
{
	...
	if (aud_tras_drv_info.spk_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
		...
		if (ring_buffer_get_fill_size(aud_tras_drv_info.spk_info.spk_rx_rb) >= aud_tras_drv_info.spk_info.uac_spk_buff_size) {
			size = ring_buffer_read(aud_tras_drv_info.spk_info.spk_rx_rb, (uint8_t*)aud_tras_drv_info.spk_info.uac_spk_buff, aud_tras_drv_info.spk_info.uac_spk_buff_size);
			if (size != aud_tras_drv_info.spk_info.uac_spk_buff_size) {
				LOGE("read spk_rx_ring_buff fail, size:%d \r\n", size);
				os_memset(aud_tras_drv_info.spk_info.uac_spk_buff, 0, aud_tras_drv_info.spk_info.uac_spk_buff_size);
			}
		} else {
			LOGW("spk_rx_rb is empty \r\n");
			os_memset(aud_tras_drv_info.spk_info.uac_spk_buff, 0, aud_tras_drv_info.spk_info.uac_spk_buff_size);
...
static bk_err_t aud_tras_drv_spk_init(aud_intf_spk_config_t *spk_cfg)
{
	...
	if (aud_tras_drv_info.spk_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
		...
		if (aud_tras_drv_info.spk_info.uac_config == NULL) {
			...
			if (aud_tras_drv_info.spk_info.uac_config == NULL) {
				...
				aud_tras_drv_info.spk_info.uac_config->mic_format_tag = AUD_UAC_DATA_FORMAT_TYPE_PCM;
				aud_tras_drv_info.spk_info.uac_config->spk_format_tag = AUD_UAC_DATA_FORMAT_TYPE_PCM;
				aud_tras_drv_info.spk_info.uac_config->mic_samp_rate = 8000;
				aud_tras_drv_info.spk_info.uac_config->spk_samp_rate = 8000;
				aud_tras_drv_info.spk_info.uac_config->spk_volume = 0;
			}
			aud_tras_drv_info.spk_info.uac_config->spk_samp_rate = spk_cfg->samp_rate;
			aud_tras_drv_info.spk_info.uac_config->spk_volume = spk_cfg->spk_gain;
		}
	}
	...
	if (aud_tras_drv_info.spk_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
...
#endif
		ret = bk_aud_uac_set_param(aud_tras_drv_info.spk_info.uac_config);
		if (ret != BK_OK) {
		...
		aud_tras_drv_set_spk_gain(aud_tras_drv_info.spk_info.uac_config->spk_volume);
...
static bk_err_t aud_tras_drv_mic_init(aud_intf_mic_config_t *mic_cfg)
{
	...
	if (aud_tras_drv_info.mic_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		...
		if (aud_tras_drv_info.mic_info.uac_config == NULL) {
			...
			if (aud_tras_drv_info.mic_info.uac_config == NULL) {
				...
				aud_tras_drv_info.mic_info.uac_config->mic_format_tag = AUD_UAC_DATA_FORMAT_TYPE_PCM;
				aud_tras_drv_info.mic_info.uac_config->spk_format_tag = AUD_UAC_DATA_FORMAT_TYPE_PCM;
				aud_tras_drv_info.mic_info.uac_config->mic_samp_rate = 8000;
				aud_tras_drv_info.mic_info.uac_config->spk_samp_rate = 8000;
				aud_tras_drv_info.mic_info.uac_config->spk_volume = 0;
			}
	...
	if (aud_tras_drv_info.mic_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		...
		if (aud_tras_drv_info.mic_info.mic_mode == AUD_ADC_MODE_SIGNAL_END) {
			ret = bk_aud_adc_set_mic_mode(AUD_MIC_MIC1, AUD_ADC_MODE_SIGNAL_END);
			if (ret != BK_OK) {
...
aud_tras_drv_mic_init_exit:
	...
	aud_tras_drv_info.mic_info.mic_mode = AUD_ADC_MODE_DIFFEN;
...
static bk_err_t aud_tras_drv_mic_deinit(void)
{
	if (aud_tras_drv_info.mic_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		...
		bk_dma_free(DMA_DEV_AUDIO, aud_tras_drv_info.mic_info.mic_dma_id);
		bk_aud_adc_deinit();
		bk_aud_adc_set_mic_mode(AUD_MIC_BOTH, AUD_ADC_MODE_DIFFEN);
	...
	if (aud_tras_drv_info.voc_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		...
		if (aud_tras_drv_info.voc_info.uac_config == NULL) {
			...
			if (aud_tras_drv_info.voc_info.uac_config == NULL) {
				...
				aud_tras_drv_info.voc_info.uac_config->mic_format_tag = AUD_UAC_DATA_FORMAT_TYPE_PCM;
				aud_tras_drv_info.voc_info.uac_config->spk_format_tag = AUD_UAC_DATA_FORMAT_TYPE_PCM;
				aud_tras_drv_info.voc_info.uac_config->mic_samp_rate = 8000;
				aud_tras_drv_info.voc_info.uac_config->spk_samp_rate = 8000;
				aud_tras_drv_info.voc_info.uac_config->spk_volume = 0;
			}
			aud_tras_drv_info.voc_info.uac_config->mic_samp_rate = voc_cfg->samp_rate;
			aud_tras_drv_info.voc_info.uac_config->spk_samp_rate = voc_cfg->samp_rate;
			aud_tras_drv_info.voc_info.uac_config->spk_volume = voc_cfg->aud_setup.dac_gain;
		}
	}
...
static bk_err_t aud_tras_drv_voc_start(void)
{
	...
	if (aud_tras_drv_info.voc_info.mic_en == AUD_INTF_VOC_MIC_OPEN) {
		if (aud_tras_drv_info.voc_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
			...
			if (aud_tras_drv_info.voc_info.mic_mode == AUD_ADC_MODE_SIGNAL_END) {
				ret = bk_aud_adc_set_mic_mode(AUD_MIC_MIC1, AUD_ADC_MODE_SIGNAL_END);
				if (ret != BK_OK) {
					LOGE("audio adc intf mode fail \r\n");
					err = BK_ERR_AUD_INTF_ADC;
...
//	if (samp_rate != AUD_DAC_SAMP_RATE_MAX) {
		bk_aud_dac_stop();
		aud_tras_dac_pa_ctrl(false);
		bk_aud_dac_set_samp_rate(samp_rate);
		bk_aud_dac_start();
		bk_aud_dac_start();
		aud_tras_dac_pa_ctrl(true);
		...
		if (kNoErr == ret) {
			switch (msg.op) {
				...
				case AUD_TRAS_DRV_MIC_SET_SAMP_RATE:
					...
					aud_tras_drv_mic_set_samp_rate(*((uint32_t *)msg.param));
					break;
				...
				case AUD_TRAS_DRV_SPK_SET_SAMP_RATE:
					LOGD("goto: AUD_TRAS_DRV_SPK_STOP \r\n");
					aud_tras_drv_spk_set_samp_rate(*((uint32_t *)msg.param));
					break;
